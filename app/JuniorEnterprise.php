...
class JuniorEnterprise extends Model
{
    ...
    {
        return DB::table('junior_enterprises as ej')
        ->selectRaw('ej.id as id_ej, ej.name as nome_ej, cores.color as core_color, cores.name as nome_nucleo, foundations.name as ies,junior_enterprise_goals.cluster as cluster, format(SUM(projects.billing),2) as soma_fat, SUM(projects.project_quantity) as soma_proj, format(junior_enterprise_goals.billing,2) as meta_fat,  junior_enterprise_goals.projects as meta_proj, truncate((sum(projects.billing) / (junior_enterprise_goals.billing) * 100),6) as porc_fat, truncate((sum(projects.project_quantity) / (junior_enterprise_goals.projects) * 100),6) as porc_proj, junior_enterprise_goals.members_performing_goal as mem_meta, junior_enterprise_goals.members_performing as mem_fat, truncate(((junior_enterprise_goals.members_performing) / (junior_enterprise_goals.members_performing_goal) * 100),6) as porc_mem, junior_enterprise_goals.shared_actions as meta_con, junior_enterprise_goals.current_shared_actions as soma_con, truncate((junior_enterprise_goals.current_shared_actions / (junior_enterprise_goals.shared_actions)* 100),6) as porc_con, junior_enterprise_goals.members_events as meta_eve, junior_enterprise_goals.current_members_events as soma_eve, truncate(((junior_enterprise_goals.current_members_events) / (junior_enterprise_goals.members_events) * 100),6) as porc_eve, junior_enterprise_goals.nps as meta_nps, junior_enterprise_goals.current_nps as soma_nps,truncate(((junior_enterprise_goals.nps) / (junior_enterprise_goals.current_nps) * 100),6) as porc_nps, (junior_enterprise_goals.current_impact_projects) as soma_impacto, (junior_enterprise_goals.impact_projects) as meta_impacto, truncate(((junior_enterprise_goals.current_impact_projects) / (junior_enterprise_goals.impact_projects) * 100),6) as porc_impacto')  
